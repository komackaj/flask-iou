...
class OfferTest(TestBase):
    ...
    def check_credit(self, expected):
        loggedUser = self.call('/api/user/current')
    ...
    def test_create_offer_without_target(self):
        ...
        with self.client:
            self.loginAsAdmin()
            offer = self.createOffer(self.adminId, 'stone', amount=1, price=15)
            check(self.adminId, offer)
    ...
    def test_create_accept_offer(self):
        with self.client:
            ...
            ownerId = self.createUser(ownerEmail)['id']
            targetId = self.createUser(targetEmail)['id']
    ...
    def test_accept_offer_without_target_has_target_in_transaction(self):
        with self.client:
            ownerId = self.createUser('owner_AOWT@test.com')['id']
            offer = self.createOffer(ownerId, 'tapas', amount=5, price=3)
            self.assertIsNone(offer['target'])
            offerUrl = '/api/offer/{}'.format(offer['id'])

            targetId = self.createUser('target_AOWT@test.com')['id']
    ...
    def test_accept_by_nontarget_denied(self):
        with self.client:
            targetId = self.createUser('target@test.com')['id']
    ...
    def test_accept_without_target(self):
        with self.client:
            self.loginAsAdmin()
            offer = self.createOffer(self.adminId, 'pizza', amount=1, price=17)
            self.createUser('another_AWT@test.com')
    ...
    def test_decline(self):
        ...
        with self.client:
            ...
            targetId = self.login(targetEmail)['id']
            offer = self.createOffer(ownerId, 'pizza', amount=1, price=15, targetId=targetId)
            offerUrl = '/api/offer/{}'.format(offer['id'])
    ...
    def test_decline_by_nontarget_denied(self):
         with self.client:
            targetId = self.createUser('targetDeclineDenied@test.com')['id']
            ownerId = self.createUser('adminDeclineDenied@test.com')['id']
    ...
    def test_remove(self):
        with self.client:
            ownerId = self.createUser('adminRemove@test.com')['id']
    ...
    def test_autoaccept_admin(self):
        with self.client:
            userId = self.createUser('user_AAA@test.com')['id']
            oldCredit = self.loginAsAdmin()['credit']
            transaction = self.createOffer(self.adminId, 'fee', amount=1, price=8, targetId=userId)
            self.assertIn('timestamp', transaction)
            self.check_credit(oldCredit + 8)
